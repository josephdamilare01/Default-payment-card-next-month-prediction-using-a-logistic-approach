...
#ggplot(ucl_decoded, aes(x = BILL_AMT2  ,  y= ..density.., fill =
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x = BILL_AMT3  ,  y= ..density.., fill =
                          #default.payment.next.month)) +geom_density()
...
#ggplot(ucl_decoded, aes(x = BILL_AMT4 , y= ..density.., fill = 
                          #default.payment.next.month)) +geom_density()
...
#ggplot(ucl_decoded, aes(x = BILL_AMT5 , y= ..density.., fill = 
                          #default.payment.next.month)) +geom_density()
...
#ggplot(ucl_decoded, aes(x = BILL_AMT6  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT1  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT2  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT3  , y= ..density.., fill = 
#default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT4  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT5  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
#ggplot(ucl_decoded, aes(x= PAY_AMT6  , y= ..density.., fill = 
                          #default.payment.next.month))  +geom_density()
...
summary(selected)
xtable(ucl_decoded, SEX)
categorical_data <- ucl_decoded %>% select(SEX, EDUCATION, MARRIAGE, )
...
LPM_predict <- predict(LPM, newdata = Train)
# Assuming you have already fitted the logistic regression model 'logit'
# Assuming you have already trained your logistic regression model (Logit_2) and have a test dataset (Test_2)
...
plot_model(LPM,show.values = T)
plot_residuals(LPM)
logit_pred <- predict(logit, newdata = test)
# Assuming you have already fitted the logistic regression model 'logit'
...
# Assuming Train contains your training data
...
# Create a dataframe combining original data and predicted values
data_from_LPM_model <- cbind(Train, predicted = LPM_predict)

# Load ggplot2 package


# For example, let's visualize the relationship between a predictor (e.g., AGE) and the predicted outcome
ggplot(data_from_LPM_model, aes(x = AGE, y = predicted)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Age", y = "Predicted default.payment.next.month")
...
# Fit the linear regression model
LPM <- lm(default.payment.next.month ~ ., data = Train_2)
